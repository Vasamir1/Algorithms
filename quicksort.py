...
def quicksort(arr):
    ...
    if len(arr) < 2:
        ...
        #  Now create array with smaller numbers
        less_than_piv = [i for i in arr[1:] if i <= piv]

        #  And array with bigger numbers
        more_than_piv = [i for i in arr[1:] if i > piv]

        #  If you want to see, how is it working inside, 'uncomment' lower line:
        ...
        #  In the end we return sorted lists added to each other.
        #  Remember we are using "stack". For more info go to: https://en.wikipedia.org/wiki/Stack_(abstract_data_type)
